...
// Factorisation du nom du fichier pour les logs
...
export function getAuthStatus() {
  logger.log(FILE_NAME, `getAuthStatus() => isAuthenticated=${isAuthenticated}, hasToken=${!!authToken}`);
...
export function setAuthenticated(status, token = null) {
  logger.log(FILE_NAME, `setAuthenticated(${status}, ${token ? `token ${token.substring(0, 4)}...${token.substring(token.length-4)}` : 'pas de token'})`);
...
function broadcastAuthStatus() {
  ...
  logger.log(FILE_NAME, `Diffusion du statut d'authentification: ${JSON.stringify(status)}`);
  ...
  try {
    chrome.runtime.sendMessage({ 
      ...
      if (chrome.runtime.lastError) {
        logger.warn(FILE_NAME, `Message authStatusChanged non délivré: ${chrome.runtime.lastError.message}`);
      } else {
        logger.log(FILE_NAME, 'Message authStatusChanged délivré avec succès');
      }
    });
  } catch (error) {
    logger.error(FILE_NAME, `Erreur lors de la diffusion du statut: ${error.message}`);
  }
}
...
export function loadAuthState() {
  logger.log(FILE_NAME, 'Chargement du statut d\'authentification depuis le stockage local...');
    ...
    if (token) {
      ...
      setTimeout(() => {
        checkAuthWithServer()
          .then(result => {
            if (result === false) {
              logger.error(FILE_NAME, 'ALERTE: Le token chargé semble invalide selon le serveur!');
              // On ne reset pas l'authentification ici pour éviter les déconnexions en cas de problème réseau
            } else {
              logger.log(FILE_NAME, 'Token chargé validé par le serveur');
            }
          })
          .catch(error => {
            logger.error(FILE_NAME, `Erreur lors de la validation du token: ${error}`);
          });
      }, 500); // Attendre que les services soient prêts
    } else {
      isAuthenticated = false;
      authToken = null;
      logger.log(FILE_NAME, 'Aucune session authentifiée trouvée dans le stockage local');
    }
...
export async function loginToServer(password) {
  ...
  chrome.storage.session.remove('authTokenBackup', () => {
    if (chrome.runtime.lastError) {
      logger.warn(FILE_NAME, `Erreur lors de la suppression du token de session: ${chrome.runtime.lastError.message}`);
    }
  });
  ...
  if (result.success) {
    ...
    try {
      ...
      logger.log(FILE_NAME, 'Token sauvegardé en session storage (sauvegarde rapide)');
    } catch (e) {
      logger.warn(FILE_NAME, `Impossible de sauvegarder en session storage: ${e.message}`);
    }
    ...
    try {
      ...
      logger.error(FILE_NAME, `Échec de sauvegarde du token dans le stockage local: ${storageError.message}`);
    ...
    if (!headers.Authorization) {
      logger.error(FILE_NAME, 'ERREUR CRITIQUE: Token non disponible dans les headers après connexion!');
    } else {
      logger.log(FILE_NAME, `Vérification des headers après login: ${JSON.stringify(headers)}`);
    }
  }
...
export async function checkAuthWithServer() {
  ...
  if (!headers.Authorization && isAuthenticated) {
    ...
    logger.error(FILE_NAME, 'ERREUR CRITIQUE: Marqué comme authentifié mais aucun token disponible!');
    
    // Essayer de récupérer depuis le stockage de secours
    const backupToken = await getSessionToken();
    if (backupToken) {
      logger.warn(FILE_NAME, 'Récupération du token depuis le stockage de secours');
      authToken = backupToken;
      return checkAuthWithServer(); // Réessayer avec le token récupéré
    } else {
      // Pas de sauvegarde disponible non plus
      logger.error(FILE_NAME, 'Aucun token de sauvegarde disponible, déconnexion forcée');
      resetAuthentication();
      return false;
    }
  }
  ...
  if (result === true || result === false) {
    ...
    if (result !== isAuthenticated) {
      logger.log(FILE_NAME, `Mise à jour du statut local: ${isAuthenticated} -> ${result}`);
      if (result) {
    ...
    logger.warn(FILE_NAME, 'Token invalide ou expiré, déconnexion forcée');
    resetAuthentication();
    return false;
    ...
    logger.error(FILE_NAME, `Erreur lors de la vérification: ${result.message}`);
    return isAuthenticated;
  }
...
export function getAuthHeaders() {
  if (!authToken) {
    logger.warn(FILE_NAME, 'WARNING: getAuthHeaders appelé sans token disponible!');
    // Tentative de récupération synchrone depuis une variable de sauvegarde
    const backupToken = getBackupToken();
    if (backupToken) {
      logger.warn(FILE_NAME, `Récupération d'urgence du token depuis la sauvegarde de session`);
      authToken = backupToken;
      isAuthenticated = true;
    } else {
      logger.error(FILE_NAME, 'ERREUR CRITIQUE: Aucun token disponible, authentification compromise');
  ...
  logger.log(FILE_NAME, `Génération des headers d'authentification avec token: ${authToken.substring(0, 4)}...${authToken.substring(authToken.length-4)}`);
...
export async function handleTokenInconsistency() {
  logger.log(FILE_NAME, 'Début de la récupération de token');
  
  // Essayer de récupérer depuis sessionStorage
  const backupToken = getBackupToken();
  if (backupToken) {
    logger.warn(FILE_NAME, 'Récupération du token de sauvegarde');
  ...
  if (storedToken) {
    logger.warn(FILE_NAME, 'Récupération du token stocké localement');
    return setToken(storedToken);
  }
  
  // Aucun token disponible
  logger.error(FILE_NAME, 'Aucun token disponible, déconnexion forcée');
...
export function resetAuthentication() {
  logger.log(FILE_NAME, 'Réinitialisation forcée de l\'authentification');
  isAuthenticated = false;
  authToken = null;
...
export function setToken(token) {
  if (!token) {
    logger.error(FILE_NAME, 'Tentative de définition d\'un token nul ou vide!');
    return false;
  }
  
  logger.log(FILE_NAME, `Définition directe du token: ${token.substring(0, 4)}...${token.substring(token.length-4)}`);
  authToken = token;
  isAuthenticated = true;
